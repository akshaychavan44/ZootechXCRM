...
export type LocalInvoiceItem = { id?: string; name: string; hsn?: string; qty: number; unit?: string; rate: number; discount?: number; gst?: number };
export type LocalInvoice = { id: string; invoice_number: string; client_id: string; client_name: string; total: number; paid_amount: number; due_date: string; items?: LocalInvoiceItem[]; created_by_id?: string; created_by_name?: string; created_at: string };
